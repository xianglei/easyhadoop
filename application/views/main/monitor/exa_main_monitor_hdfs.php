...
function datanode_use(host_id)
{
	$.getJSON('<?php echo $this->config->base_url();?>index.php/monitor/datanodestats/' + host_id, function(json){
		...
		for(var key in json.volume_info){
			...
			html += '<tr><td><small><?php echo $common_mount_point;?>: </small></td><td><small><?php echo $common_hdfs_free;?>: </small></td><td><small><?php echo $common_hdfs_used;?>: </small></td><td><small><?php echo $common_hdfs_reserved;?>: </small></td><td><?php echo $common_hdd_status?></td></tr>';
			...
			html += '<div class="progress-bar progress-bar-success" style="width: ' + freePer + '%;">Free</div>';
			html += '<div class="progress-bar progress-bar-warning" style="width: ' + reservedPer + '%;">Reserved</div>';
			html += '<div class="progress-bar progress-bar-danger" style="width: ' + usedPer + '%;">DFS</div>';
			html += '</div>'
			html += '</td>';
			html += '</tr>';
		}
...
<div class="progress" id="namenode_progress">
	<div class="progress-bar progress-bar-success" style="" id="namenode_progress_free">Free</div>
	<div class="progress-bar progress-bar-warning" style="" id="namenode_progress_nondfs">NonDFS</div>
	<div class="progress-bar progress-bar-danger" style="" id="namenode_progress_dfs">DFS</div>
</div>
...
<table class="table table-striped">
	...
	<?php $i=1; foreach($results as $item):?>
		<tr>
			<td><?php echo $i;?></td>
			<td><a data-toggle="modal" href="#datanode_modal_<?php echo $item->id;?>"><?php echo $item->hostname;?></a></td>
			<td><?php echo $item->ip;?></td>
			<td>
			<div class="progress">
				<div class="progress-bar progress-bar-success" style="" id="datanode_free_<?php echo $item->id;?>">Free</div>
				<div class="progress-bar progress-bar-danger" style="" id="datanode_dfs_<?php echo $item->id;?>">DFS</div>
			<script>
			
			datanode_use(<?php echo $item->id;?>);
			setInterval(function()
			{
				datanode_use(<?php echo $item->id;?>)
			}, 30000);
			...
			<div id="datanode_dfs_use_detail_<?php echo $item->id;?>">
			</div>
			<!--modaled detail node hdd usage-->
			<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" id="datanode_modal_<?php echo $item->id?>">
			...
			<div class="modal-content">
				...
				<div class="modal-body" id="datanode_modal_mountpoint_<?php echo $item->id;?>">
				</div>
